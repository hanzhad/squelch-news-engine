...
module turns the same PUBLISHED issues into a single static page plus an RSS
feed, both of which GitHub Pages can serve straight out of the repository.
...
FEED_FILE = "rss.xml"
# Readers keep the whole feed in memory; the page keeps the whole archive.
FEED_LIMIT = 50
...
def _published_at(issue: IssueRecord) -> datetime:
    ...
    candidates = (
        ...
        issue.meta.get("published_at_discord"),
        issue.created_at,
    )
...
def _write_feed(
    ...
    for article in articles[:FEED_LIMIT]:
        if not article.url:
...
def build_site(settings: Settings, config: Config, store: IssueStore, out_dir: Path) -> None:
    ...
    _write_feed(articles, config.title, tagline, base_url, out_dir / FEED_FILE)
